...
const Uniform: React.FC = () => {
  ...
  const handleWhatsAppClick = (): void => {
    const message = `Hi! I'm interested in ordering T-Shirts & Uniforms. Can you provide more information about:

1. Apparel Type: [T-Shirts/Corporate Uniforms/Other]
2. Quantity Needed: ${quantity} pieces
3. Sizes Required: [Breakdown by size]
4. Printing Method: [Screen Print/Embroidery/Digital Print]
5. Fabric Type/Quality:
6. Design Requirements:
7. Color Preferences:
8. Timeline:
9. Budget Range:

Please provide details about available uniform options, pricing for ${quantity} pieces, sample availability, production timeline, and any bulk order discounts.`;
          ...
          <div className="flex gap-3 md:gap-4 justify-center overflow-x-auto !py-2 !px-4 md:hidden">
            {images.map((img, index) => (
              <button
                ...
                aria-label={`View uniform design ${index + 1}`}
              >
                <img 
                  src={img} 
                  alt={`Uniform Design ${index + 1}`} 
          ...
          <div className="space-y-4 !pt-4 font-[poppins]">
            ...
            <div className="space-y-4 !pt-6">
              ...
              >
                ...
                <span className="text-lg md:text-xl !ml-2">Order {quantity} Uniform{quantity > 1 ? 's' : ''}</span>
              </button>
            </div>
          </div>
        </div>
