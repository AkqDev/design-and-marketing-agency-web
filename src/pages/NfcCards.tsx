...
const NfcCards: React.FC = () => {
  ...
  const product = {
    title: "NFC Cards",
    description: "Step into the future with smart NFC cards. Combine traditional business cards with modern technology for interactive experiences. Instantly share contact details, websites, social media profiles, and digital content with a simple tap on any NFC-enabled smartphone.",
    features: [
      "PVC NFC Cards",
  ...
  const handleWhatsAppClick = (): void => {
    const message = `Hi! I'm interested in NFC cards. Can you provide more information about:

1. Card Type: [PVC NFC Cards/Metal NFC Cards]
2. Quantity Needed: ${quantity} cards
3. Card Design Requirements:
4. NFC Chip Type: [Standard/Premium]
5. Programming Needs: [Contact Info/Website/Social Media/Other]
6. Packaging Requirements:
7. Timeline:
8. Budget Range:

Please provide details about available NFC card options, programming services, pricing for ${quantity} cards, sample availability, production timeline, and compatibility information.`;
          ...
          <div className="flex gap-3 md:gap-4 justify-center overflow-x-auto !py-2 !px-4 md:hidden">
            {images.map((img, index) => (
              <button
                ...
                aria-label={`View NFC card design ${index + 1}`}
              >
                <img 
                  src={img} 
                  alt={`NFC Card Design ${index + 1}`} 
          ...
          <div className="space-y-4 !pt-4 font-[poppins]">
            ...
            <div className="space-y-4 !pt-6">
              ...
              >
                ...
                <span className="text-lg md:text-xl !ml-2">Order {quantity} NFC Card{quantity > 1 ? 's' : ''}</span>
              </button>
            </div>
          </div>
        </div>
