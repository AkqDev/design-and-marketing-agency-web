...
const VisitingCards: React.FC = () => {
  const images = [visitingcard1, visitingcard2, visitingcard3, visitingcard4];
  const [activeImage, setActiveImage] = useState<number>(0);
  const [quantity, setQuantity] = useState<number>(1);
  
  const product = {
    title: "Visiting Cards",
    description: "Make a lasting impression with our premium visiting cards. We offer a range of finishes and materials to match your professional identity. Perfect for networking events, client meetings, and business interactions that require professional representation.",
    features: [
  ...
  const handleWhatsAppClick = (): void => {
    const message = `Hi! I'm interested in ordering visiting cards. Can you provide more information about:

1. Card Type: [Simple/UV Spot/Embossed/Foil Stamping]
2. Quantity Needed: ${quantity} cards
3. Finish Preference: [Matte/Gloss]
4. Paper Quality/GSM:
5. Design Requirements:
6. Additional Features: [Rounded Corners/Spot UV/Other]
7. Timeline:
8. Budget Range:

Please provide details about available visiting card options, pricing for ${quantity} cards, sample availability, production timeline, and any minimum order requirements.`;
          ...
          <div className="flex gap-3 md:gap-4 justify-center overflow-x-auto !py-2 !px-4 md:hidden">
            {images.map((img, index) => (
              <button
                ...
                aria-label={`View visiting card design ${index + 1}`}
              >
                <img 
                  src={img} 
          ...
          <div className="space-y-4 !pt-4 font-[poppins]">
            ...
            <div className="space-y-4 !pt-6">
              ...
              >
                ...
                <span className="text-lg md:text-xl !ml-2">Order {quantity} Visiting Card{quantity > 1 ? 's' : ''}</span>
              </button>
            </div>
          </div>
        </div>
