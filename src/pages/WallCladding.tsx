...
const WallCladding: React.FC = () => {
  const images = [cladding1, cladding2, cladding3, cladding4];
  const [activeImage, setActiveImage] = useState<number>(0);
  const [quantity, setQuantity] = useState<number>(1);
  
  const product = {
    title: "Wall Cladding Services",
    description: "Transform your spaces with premium wall cladding solutions. We provide durable, stylish, and weather-resistant cladding materials perfect for interior and exterior applications. Enhance aesthetics and protection for residential, commercial, and industrial properties.",
    features: [
  ...
  const handleWhatsAppClick = (): void => {
    const message = `Hi! I'm interested in wall cladding services. Can you provide more information about:

1. Application Area: [Interior/Exterior/Both]
2. Area Size/Sq. Meters:
3. Material Preference: [PVC/Wood/Metal/Composite/Other]
4. Design/Style Requirements:
5. Installation Needed: [Yes/No]
6. Location Details:
7. Timeline:
8. Budget Range:

Please provide details about available wall cladding materials, pricing per square meter, sample availability, installation services, and any warranty information.`;
    ...
    <div className="min-h-screen w-full flex items-center justify-center !p-4 md:!p-6 font-sans text-gray-900 !my-12">
      ...
      <div className="max-w-[1200px] w-full grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center bg-[#1A1A1A] rounded-3xl !p-6 md:!p-8  shadow-[#9d0a0a]/35 shadow-xl">
          ...
          <div className="flex gap-3 md:gap-4 justify-center overflow-x-auto !py-2 !px-4 ">
            {images.map((img, index) => (
              <button
                ...
                aria-label={`View wall cladding design ${index + 1}`}
              >
                <img 
                  src={img} 
                  alt={`Wall Cladding Design ${index + 1}`} 
        ...
        <div className="flex flex-col justify-center space-y-6 md:space-y-8 !py-4 text-center md:text-left">
          ...
          {/* Quantity Selector - Changed to Area/Sq. Meters */}
